...
import * as AasConstants from './constants.js';
 ...
 */
export function langStringsHaveUniqueLanguages(langStrings: Iterable<AasTypes.IAbstractLangString>): boolean {
 ...
 * @returns `true` if the check passes
 */
export function qualifierTypesAreUnique(qualifiers: Iterable<AasTypes.Qualifier>): boolean {
...
export function isXsLong(value: string): boolean {
    ...
    const limit = match[1] === '-' ? SMALLEST_LONG_WITHOUT_SIGN_AS_STRING : LARGEST_LONG_AS_STRING;
...
const DATA_TYPE_DEF_XSD_TO_VALUE_CONSISTENCY = new Map<AasTypes.DataTypeDefXsd, (string) => boolean>([
 ...
 */
export function valueConsistentWithXsdType(value: string, valueType: AasTypes.DataTypeDefXsd): boolean {
    const verifier = DATA_TYPE_DEF_XSD_TO_VALUE_CONSISTENCY.get(valueType);
    if (verifier === undefined) {
        throw new Error('The value type is invalid. Expected a literal of DataTypeDefXsd, ' + `but got: ${valueType}`);
    }
 ...
 */
export function isModelReferenceTo(reference: AasTypes.Reference, expectedType: AasTypes.KeyTypes): boolean {
    return (
 ...
 * @returns `true` if the check passes
 */
export function extensionNamesAreUnique(extensions: Iterable<AasTypes.Extension>): boolean {
 ...
 */
export function submodelElementsHaveIdenticalSemanticIds(elements: Iterable<AasTypes.ISubmodelElement>): boolean {
...
const AAS_SUBMODEL_ELEMENTS_TO_IS = new Map<AasTypes.AasSubmodelElements, (that: AasTypes.Class) => boolean>([
    [AasTypes.AasSubmodelElements.AnnotatedRelationshipElement, AasTypes.isAnnotatedRelationshipElement],
    ...
    [AasTypes.AasSubmodelElements.MultiLanguageProperty, AasTypes.isMultiLanguageProperty],
    ...
    [AasTypes.AasSubmodelElements.SubmodelElementCollection, AasTypes.isSubmodelElementCollection],
]);
 ...
 */
export function referenceKeyValuesEqual(that: AasTypes.Reference, other: AasTypes.Reference): boolean {
 ...
 */
class Verifier extends AasTypes.AbstractTransformerWithContext<boolean, IterableIterator<VerificationError>> {
    *transformExtensionWithContext(that: AasTypes.Extension, context: boolean): IterableIterator<VerificationError> {
        ...
        if (!(!(that.refersTo !== null) || that.refersTo.length >= 1)) {
            yield new VerificationError('Refers-to must be either not set or have at least one item.');
        }
            ...
            if (that.supplementalSemanticIds !== null) {
                ...
                for (const item of that.supplementalSemanticIds) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(
                            new IndexSegment(that.supplementalSemanticIds, supplementalSemanticIdsIndex),
                        );
        ...
        if (context === true) {
            if (that.embeddedDataSpecifications !== null) {
                ...
                for (const item of that.embeddedDataSpecifications) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(
                            new IndexSegment(that.embeddedDataSpecifications, embeddedDataSpecificationsIndex),
                        );
    ...
    *transformQualifierWithContext(that: AasTypes.Qualifier, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.supplementalSemanticIds !== null) || that.supplementalSemanticIds.length >= 1)) {
            yield new VerificationError(
                'Supplemental semantic IDs must be either not set or have at ' + 'least one item.',
            );
    ...
    *transformAssetAdministrationShellWithContext(
        ...
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
        ...
        if (!(!(that.description !== null) || that.description.length >= 1)) {
            yield new VerificationError('Description must be either not set or have at least one ' + 'item.');
        }
        ...
        if (!(!(that.displayName !== null) || that.displayName.length >= 1)) {
            yield new VerificationError('Display name must be either not set or have at least one ' + 'item.');
        }
        ...
        if (!(!(that.submodels !== null) || that.submodels.length >= 1)) {
            yield new VerificationError('Submodels must be either not set or have at least one item.');
        }
        ...
        if (
            ...
            yield new VerificationError('All submodels must be model references to a submodel.');
        }
    ...
    *transformAssetInformationWithContext(
        ...
        if (!(!(that.specificAssetIds !== null) || that.specificAssetIds.length >= 1)) {
            yield new VerificationError('Specific asset IDs must be either not set or have at least ' + 'one item.');
        }
        ...
        if (context === true) {
            if (that.specificAssetIds !== null) {
                ...
                for (const item of that.specificAssetIds) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(new IndexSegment(that.specificAssetIds, specificAssetIdsIndex));
    ...
    *transformSubmodelWithContext(that: AasTypes.Submodel, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
        ...
        if (!(!(that.qualifiers !== null) || that.qualifiers.length >= 1)) {
            yield new VerificationError('Qualifiers must be either not set or have at least one item.');
        }
        ...
        if (!(!(that.submodelElements !== null) || that.submodelElements.length >= 1)) {
            yield new VerificationError('Submodel elements must be either not set or have at least ' + 'one item.');
        }

        if (
            !(
                !(that.submodelElements !== null) ||
                AasCommon.every(AasCommon.map(that.submodelElements, item => item.idShort !== null))
            )
        ) {
            yield new VerificationError(
                        ...
                        submodelElement =>
                            ...
                            AasCommon.every(
                                AasCommon.map(
                                    ...
                                    qualifier => qualifier.kindOrDefault() != AasTypes.QualifierKind.TemplateQualifier,
                                ),
                            ),
                    ),
                )
        ...
        if (context === true) {
            ...
            if (that.submodelElements !== null) {
                ...
                for (const item of that.submodelElements) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(new IndexSegment(that.submodelElements, submodelElementsIndex));
    ...
    *transformSubmodelElementListWithContext(
        ...
        if (!(!(that.value !== null) || that.value.length >= 1)) {
            yield new VerificationError('Value must be either not set or have at least one item.');
        }
        ...
        if (
            !(
                !(that.value !== null) ||
                AasCommon.every(
                    AasCommon.map(that.value, element => submodelElementIsOfType(element, that.typeValueListElement)),
                )
            )
        ) {
            yield new VerificationError(
        ...
        if (
            !(
                ...
                (that.valueTypeListElement !== null &&
                    (that.value === null || propertiesOrRangesHaveValueType(that.value, that.valueTypeListElement)))
            )
        ) {
            yield new VerificationError(
        ...
        if (
            !(!(that.value !== null) || AasCommon.every(AasCommon.map(that.value, element => element.idShort === null)))
        ) {
            yield new VerificationError(
        ...
        if (context === true) {
            ...
            if (that.semanticIdListElement !== null) {
                for (const error of this.transformWithContext(that.semanticIdListElement, context)) {
    ...
    *transformSubmodelElementCollectionWithContext(
        ...
        if (!(!(that.value !== null) || AasCommon.every(AasCommon.map(that.value, item => item.idShort !== null)))) {
            yield new VerificationError(
    ...
    *transformPropertyWithContext(that: AasTypes.Property, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
    ...
    *transformRangeWithContext(that: AasTypes.Range, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
    ...
    *transformBlobWithContext(that: AasTypes.Blob, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
    ...
    *transformFileWithContext(that: AasTypes.File, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
    ...
    *transformAnnotatedRelationshipElementWithContext(
        ...
        if (!(!(that.annotations !== null) || that.annotations.length >= 1)) {
            yield new VerificationError('Annotations must be either not set or have at least one ' + 'item.');
        }

        if (
            !(
                !(that.annotations !== null) ||
                AasCommon.every(AasCommon.map(that.annotations, item => item.idShort !== null))
            )
        ) {
            yield new VerificationError(
    ...
    *transformEntityWithContext(that: AasTypes.Entity, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
        ...
        if (!(!(that.statements !== null) || that.statements.length >= 1)) {
            yield new VerificationError('Statements must be either not set or have at least one item.');
        }
    ...
    *transformEventPayloadWithContext(
        ...
        if (
            ...
            yield new VerificationError('Source must be a model reference to an Event element.');
        }

        if (!isModelReferenceToReferable(that.observableReference)) {
            yield new VerificationError('Observable reference must be a model reference to ' + 'a referable.');
        }
        ...
        if (context === true) {
            ...
            for (const error of this.transformWithContext(that.observableReference, context)) {
                error.path.prepend(new PropertySegment(that, 'observableReference'));
                yield error;
            }

            if (that.observableSemanticId !== null) {
                for (const error of this.transformWithContext(that.observableSemanticId, context)) {
    ...
    *transformBasicEventElementWithContext(
        ...
        if (!(!(that.direction == AasTypes.Direction.Input) || that.maxInterval === null)) {
            yield new VerificationError('Max. interval is not applicable for input direction.');
        }
        ...
        if (!(!(that.messageBroker !== null) || isModelReferenceToReferable(that.messageBroker))) {
            yield new VerificationError('Message broker must be a model reference to a referable.');
        }
    ...
    *transformOperationWithContext(that: AasTypes.Operation, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
        ...
        if (!(!(that.inputVariables !== null) || that.inputVariables.length >= 1)) {
            yield new VerificationError('Input variables must be either not set or have at least one ' + 'item.');
        }

        if (!(!(that.outputVariables !== null) || that.outputVariables.length >= 1)) {
            yield new VerificationError('Output variables must be either not set or have at least ' + 'one item.');
        }

        if (!(!(that.inoutputVariables !== null) || that.inoutputVariables.length >= 1)) {
            yield new VerificationError('Inoutput variables must be either not set or have at least ' + 'one item.');
        }
        ...
        if (context === true) {
            ...
            if (that.inputVariables !== null) {
                ...
                for (const item of that.inputVariables) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(new IndexSegment(that.inputVariables, inputVariablesIndex));
            ...
            if (that.outputVariables !== null) {
                ...
                for (const item of that.outputVariables) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(new IndexSegment(that.outputVariables, outputVariablesIndex));
            ...
            if (that.inoutputVariables !== null) {
                ...
                for (const item of that.inoutputVariables) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(new IndexSegment(that.inoutputVariables, inoutputVariablesIndex));
    ...
    *transformCapabilityWithContext(that: AasTypes.Capability, context: boolean): IterableIterator<VerificationError> {
        if (!(!(that.extensions !== null) || that.extensions.length >= 1)) {
            yield new VerificationError('Extensions must be either not set or have at least one item.');
        }
    ...
    *transformConceptDescriptionWithContext(
        ...
        if (!(!(that.isCaseOf !== null) || that.isCaseOf.length >= 1)) {
            yield new VerificationError('Is-case-of must be either not set or have at least one item.');
        }

        if (
            !(
                !(that.embeddedDataSpecifications !== null) ||
                dataSpecificationIec61360sHaveDefinitionAtLeastInEnglish(that.embeddedDataSpecifications) ||
        ...
        if (
            !(
                !(that.category !== null && that.category == 'DOCUMENT' && that.embeddedDataSpecifications !== null) ||
                dataSpecificationIec61360sForDocumentHaveAppropriateDataType(that.embeddedDataSpecifications)
            )
        ) {
            yield new VerificationError(
        ...
        if (
            !(
                !(that.category !== null && that.category == 'REFERENCE' && that.embeddedDataSpecifications !== null) ||
                dataSpecificationIec61360sForReferenceHaveAppropriateDataType(that.embeddedDataSpecifications)
            )
        ) {
            yield new VerificationError(
        ...
        if (
            !(
                ...
                dataSpecificationIec61360sForPropertyOrValueHaveAppropriateDataType(that.embeddedDataSpecifications)
            )
        ) {
            yield new VerificationError(
    ...
    *transformReferenceWithContext(that: AasTypes.Reference, context: boolean): IterableIterator<VerificationError> {
        ...
        if (
            !(
                !(that.type == AasTypes.ReferenceTypes.ExternalReference && that.keys.length >= 1) ||
            ...
            !(
                ...
                AasCommon.every(
                    AasCommon.map(
                        ...
                        i => !AasConstants.GENERIC_FRAGMENT_KEYS.has(AasCommon.at(that.keys, i).type),
                    ),
                )
            )
        ) {
        ...
        if (context === true) {
            if (that.referredSemanticId !== null) {
                for (const error of this.transformWithContext(that.referredSemanticId, context)) {
    ...
    *transformLangStringNameTypeWithContext(
        ...
        if (!(that.text.length <= 128)) {
            yield new VerificationError('String shall have a maximum length of 128 characters.');
        }
    ...
    *transformLangStringTextTypeWithContext(
        ...
        if (!(that.text.length <= 1023)) {
            yield new VerificationError('String shall have a maximum length of 1023 characters.');
        }
    ...
    *transformEnvironmentWithContext(
        ...
        if (!(!(that.conceptDescriptions !== null) || that.conceptDescriptions.length >= 1)) {
            yield new VerificationError('Concept descriptions must be either not set or have at ' + 'least one item.');
        }
        ...
        if (context === true) {
            if (that.assetAdministrationShells !== null) {
                ...
                for (const item of that.assetAdministrationShells) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(
                            new IndexSegment(that.assetAdministrationShells, assetAdministrationShellsIndex),
                        );
            ...
            if (that.conceptDescriptions !== null) {
                ...
                for (const item of that.conceptDescriptions) {
                    for (const error of this.transformWithContext(item, context)) {
                        error.path.prepend(new IndexSegment(that.conceptDescriptions, conceptDescriptionsIndex));
    ...
    *transformEmbeddedDataSpecificationWithContext(
        ...
        if (context === true) {
            for (const error of this.transformWithContext(that.dataSpecificationContent, context)) {
    ...
    *transformValueListWithContext(that: AasTypes.ValueList, context: boolean): IterableIterator<VerificationError> {
        if (!(that.valueReferencePairs.length >= 1)) {
            yield new VerificationError('Value reference pair types must contain at least one item.');
        }
    ...
    *transformLangStringPreferredNameTypeIec61360WithContext(
        ...
        if (!(that.text.length <= 255)) {
            yield new VerificationError('String shall have a maximum length of 255 characters.');
        }
    ...
    *transformLangStringShortNameTypeIec61360WithContext(
        ...
        if (!(that.text.length <= 18)) {
            yield new VerificationError('String shall have a maximum length of 18 characters.');
        }
    ...
    *transformDataSpecificationIec61360WithContext(
        ...
        if (
            !(
                !(that.dataType !== null && AasConstants.IEC_61360_DATA_TYPES_WITH_UNIT.has(that.dataType)) ||
                that.unit !== null ||
                that.unitId !== null
            )
        ) {
        ...
        if (!(!(that.definition !== null) || that.definition.length >= 1)) {
            yield new VerificationError('Definition must be either not set or have at least one item.');
        }
        ...
        if (!(!(that.shortName !== null) || that.shortName.length >= 1)) {
            yield new VerificationError('Short name must be either not set or have at least one item.');
        }
        ...
        if (!AasCommon.some(AasCommon.map(that.preferredName, langString => isBcp47ForEnglish(langString.language)))) {
            yield new VerificationError(
        ...
        if (that.sourceOfDefinition !== null) {
            for (const error of verifyNonEmptyXmlSerializableString(that.sourceOfDefinition)) {
 ...
 */
export function* verify(that: AasTypes.Class, recurse = true): IterableIterator<VerificationError> {
 ...
 */
export function* verifyNonEmptyXmlSerializableString(that: string): IterableIterator<VerificationError> {
...
export function* verifyIdentifier(that: string): IterableIterator<VerificationError> {
    ...
    if (!(that.length <= 2000)) {
        yield new VerificationError('Identifier shall have a maximum length of 2000 characters.');
    }
}
 ...
 * @returns errors, if any
 */
export function* verifyValueTypeIec61360(that: string): IterableIterator<VerificationError> {
    if (!matchesXmlSerializableString(that)) {
        yield new VerificationError(
            "Constraint AASd-130: An attribute with data type 'string' " +
                'shall consist of these characters only: ' +
    ...
    if (!(that.length <= 2000)) {
        yield new VerificationError('Value type IEC 61360 shall have a maximum length of 2000 ' + 'characters.');
    }
}
...
export function* verifyNameType(that: string): IterableIterator<VerificationError> {
    ...
    if (!(that.length <= 128)) {
        yield new VerificationError('Name type shall have a maximum length of 128 characters.');
    }
}
...
export function* verifyVersionType(that: string): IterableIterator<VerificationError> {
    ...
    if (!(that.length <= 4)) {
        yield new VerificationError('Version type shall have a maximum length of 4 characters.');
    }
}
...
export function* verifyRevisionType(that: string): IterableIterator<VerificationError> {
    ...
    if (!(that.length <= 4)) {
        yield new VerificationError('Revision type shall have a maximum length of 4 characters.');
    }
}
...
export function* verifyLabelType(that: string): IterableIterator<VerificationError> {
    ...
    if (!(that.length <= 64)) {
        yield new VerificationError('Label type shall have a maximum length of 64 characters.');
    }
}
 ...
 * @returns errors, if any
 */
export function* verifyMessageTopicType(that: string): IterableIterator<VerificationError> {
    if (!matchesXmlSerializableString(that)) {
        yield new VerificationError(
            "Constraint AASd-130: An attribute with data type 'string' " +
                'shall consist of these characters only: ' +
    ...
    if (!(that.length <= 255)) {
        yield new VerificationError('Message topic type shall have a maximum length of 255 ' + 'characters.');
    }
}
 ...
 * @returns errors, if any
 */
export function* verifyBcp47LanguageTag(that: string): IterableIterator<VerificationError> {
    if (!matchesBcp47(that)) {
        yield new VerificationError('The value must represent a value language tag conformant to ' + 'BCP 47.');
    }
}
...
export function* verifyContentType(that: string): IterableIterator<VerificationError> {
    ...
    if (!(that.length <= 100)) {
        yield new VerificationError('Content type shall have a maximum length of 100 characters.');
    }

    if (!matchesMimeType(that)) {
        yield new VerificationError('The value must represent a valid content MIME type ' + 'according to RFC 2046.');
    }
}
 ...
 * @returns errors, if any
 */
export function* verifyQualifierType(that: string): IterableIterator<VerificationError> {
    if (!matchesXmlSerializableString(that)) {
        yield new VerificationError(
            "Constraint AASd-130: An attribute with data type 'string' " +
                'shall consist of these characters only: ' +
