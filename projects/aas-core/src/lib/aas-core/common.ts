 ...
 */
export function* map<S, T>(iterable: Iterable<S>, mappingFunction: (item: S) => T): IterableIterator<T> {
    // NOTE (mristin, 2022-11-24):
    // We introduce this function so that we can keep the constraint verification
    // purely functional.
...
export function base64Decode(text: string): Either<Uint8Array, string> {
    ...
    if (bytes.length !== bytesLength) {
        throw new Error(`Expected bytes to have length ${bytesLength}, but got ${bytes.length}`);
    }
